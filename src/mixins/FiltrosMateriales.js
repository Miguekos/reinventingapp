const FiltrosMateriales = {
  computed: {
    filtrosNuevos() {
      ...
      for (let index = 0; index < array.length; index++) {
        ...
        this.status.push(element.no_catego);
        this.city.push(element.no_empres);
        this.mobil.push(element.nu_anofab);
      }
      this.$q.loading.hide();
    },
    filteredByAll() {
      const keyw = this.getByKeyword(this.getMateriales, this.keyword),
  ...
  // co_barras: "CIL0000001"
  // co_catego: 67
  // co_empres: 19
  // codigo: 7
  // no_catego: "REINVENTING S.A.C / REPUESTOS / CILINDROS DE GNV"
  // no_descri: "CILINDRO DE GNV 55 LT"
  // no_empres: "REINVENTING S.A.C"
  // va_stock: null
  data() {
    return {
      loadtable: false,
  ...
  created: function() {
    console.log("Mixin FiltrosMateriales");
  },
  methods: {
    noti(val) {
      if (val === 1) {
    ...
    getByStatus(list, status) {
      ...
      return list.filter(item => item.no_catego === status);
    },
    getByCity(list, city) {
      if (!city) return list;
      return list.filter(item => item.no_empres === city);
    },
